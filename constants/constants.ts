// APP SYMBOL -------------------------------------- //
export const SYMBOL_TEXT: string = "OURFIT";
// -------------------------------------- APP SYMBOL //

// SCREEN ------------------------------------------ //
export const SCREEN_SIZE_MB = 450;
export const SCREEN_SIZE_SM = 640;
export const SCREEN_SIZE_MD = 768; //  tablet
export const SCREEN_SIZE_XMD = 940;
export const SCREEN_SIZE_LG = 1024;
export const SCREEN_SIZE_XL = 1280;
// ------------------------------------------ SCREEN //

export const NAV_HEIGHT: number = 60;

export const VERTICAL_SIDEBAR_WIDTH = 200;

// BORDER ------------------------------------------ //
export const BORDER_TINE_WIDTH: number = 1;
export const BORDER_BASE_WIDTH: number = 1.5;
// ------------------------------------------ BORDER //

// CONTAINER INNER COMPONENT'S SCALE --------------- //
export const MAX_CONTENT_WIDTH: number = 1200;
// --------------- CONTAINER INNER COMPONENT'S SCALE //

// CONTAINER ------------------------------------------------------- //
export const CONTAINER_PADDING_VERTICAL: string = "3rem"; // 3rem
export const CONTAINER_PADDING_HORIZONTAIL: string = "2.5rem";
export const MOBILEABLE_CONTAINER_HORIZONTAL_SPACE: string =
  "w-[100%] sm:max-w-[420px]";
export const MOBILEABLE_CONTAINER_VERTICAL_SPACE: string =
  "h-[100%] sm:h-screen";
// ------------------------------------------------------- CONTAINER //

export const ICON_BUTTON_BLACK_OPACTIY: number = 50;

// Product ---------------------------------------------- //
export const PRODUCT_CARD_WIDTH = 280;
export const PRODUCT_TAB_HEIGHT = 50;
export const PRODUCT_PADDING: number = 80;
// ---------------------------------------------- Product //

// Card's ------------------------------------------------ //
export const CARDS_BORDER_RADIUS = 6; // 6px 0.375rem
export const CARDS_PADDING = 12; // 12px 0.75rem
export const CARDS_MEDIUM_PADDING = 15; // 20px
// ------------------------------------------------ Card's //
